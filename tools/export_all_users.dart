...
/// Full export of ALL users' Firestore data from the project.
///
/// Usage:
///   dart run tools/export_all_users.dart
///
/// Optional:
///   --project-id=plant-logger-e0677
///   --access-token=<token>
///   --out-dir=exports/all_users_backup
///
/// Outputs:
///   <out-dir>/users.json          — every user's full Firestore data
///   <out-dir>/photos/             — Storage images (best-effort; may be 402)
///   <out-dir>/manifest.json       — summary
const _defaultProjectId = 'plant-logger-e0677';
const _defaultOutDir = 'exports/all_users_backup';
...
Future<void> main(List<String> arguments) async {
  ...
  try {
    ...
    final users = await client.listDocuments('users');
    stdout.writeln('Найдено пользователей: ${users.length}');

    final allUsers = <String, dynamic>{};
    var photoCount = 0;
      ...
      for (final collection in _topLevelCollections) {
        ...
        for (final doc in docs) {
          ...
          if (nested.isNotEmpty) {
            userData['${collection}_$docId'] = nested;
          }
        }
      }

      allUsers[uid] = userData;
      stdout.writeln('  Экспортирован пользователь $uid');
    }

    // 4. Best-effort photo download from Storage.
    ...
    // 5. Write outputs.
    final usersFile = File('$outDir/users.json');
    ...
    final manifest = <String, dynamic>{
      ...
      'users': allUsers.keys.toList()..sort(),
    ...
    stdout.writeln(
      ...
      '  Firestore: ${usersFile.absolute.path}\n'
...
Future<bool> _downloadStorageFile(
  ...
  try {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      final body = await utf8.decoder.bind(response).join();
      stderr.writeln('  Пропуск $objectName: ${response.statusCode}');
      return false;
    }
...
class FirestoreRestClient {
  ...
  Future<List<Map<String, dynamic>>> listDocuments(String path) async {
    ...
    if (separator < 0) {
      // Top-level collection (no parent path).
      return _listCollection('', path);
    }
    final parentPath = path.substring(0, separator);
    final collectionId = path.substring(separator + 1);
    return _listCollection(parentPath, collectionId);
  }
  ...
  Future<List<Map<String, dynamic>>> _listCollection(
    ...
    do {
      ...
      final base = Uri.parse(
        parentPath.isEmpty
            ? '$_documentsBase/${Uri.encodeComponent(collectionId)}'
