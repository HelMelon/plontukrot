...
/// Card showing botanical overview and AI-generated care recommendations for a plant family.
class FamilyCareGuideCard extends StatefulWidget {
  final String family;

  const FamilyCareGuideCard({
    super.key,
    required this.family,
  });

  @override
  State<FamilyCareGuideCard> createState() => _FamilyCareGuideCardState();
}

class _FamilyCareGuideCardState extends State<FamilyCareGuideCard> {
  final FamilyCareService _service = FamilyCareService();
  Future<FamilyCareGuide?>? _careGuideFuture;
  ...
  @override
  void didUpdateWidget(covariant FamilyCareGuideCard oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.family.trim().toLowerCase() !=
        widget.family.trim().toLowerCase()) {
  ...
  void _ensureGuideLoaded({bool force = false}) {
    ...
    if (!force &&
        ...
        _loadedFamily == family.toLowerCase()) {
      return;
    }
    ...
    if (!force) {
      final cached = FamilyCareService.peekCached(family, locale: locale);
      if (cached != null) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<FamilyCareGuide?>(
          ...
          child: Column(
            ...
            children: [
              Semantics(
                button: true,
                label: _isExpanded
                    ? l10n.a11yFamilyCareCollapse
                    : l10n.a11yFamilyCareExpand,
