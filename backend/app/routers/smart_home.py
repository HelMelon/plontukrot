...
Позволяет Алисе видеть влажность горшков (сенсоры, которые ESP8266
шлёт в /sensor/reading) и строить сценарии вида «когда горшок сухой →
Алиса скажет "полей горшок"».
...
def _pot_name(pot: int) -> str:
    """Имя горшка из БД (или дефолт, если датчик ещё не присылал)."""
    with get_pool().connection() as conn:
        row = conn.execute(
            "SELECT name FROM sensor_names WHERE pot=%s", (pot,)
        ).fetchone()
    return row["name"] if row else f"Горшок {pot}"
